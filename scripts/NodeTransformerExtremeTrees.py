...
class NodeTransformerExtremeTrees( BaseEstimator, TransformerMixin ):
    #Class Constructor 
    def __init__( self, n_estimators =5,
        ...
        self.inner_tree = RandomTreesEmbedding(n_estimators=n_estimators, max_depth=max_depth, random_state=random_state)
    ...
    def fit( self, X, y = None ):
        ...
        for G in tqdm( self.graph_list):
            node2vec = Node2Vec(G, dimensions=self.dimension, walk_length=self.walk_length, num_walks=self.n_walks)
    ...
    def create_graph_list(self,estimators_trees):
        ...
        for estimator in tqdm(estimators_trees.estimators_) :
            G=nx.Graph() #
    ...
    def transform( self, X, y = None ):
        ...
        reps = []
        for estimator,model in tqdm(zip(self.inner_tree.estimators_,self.node2model)):
